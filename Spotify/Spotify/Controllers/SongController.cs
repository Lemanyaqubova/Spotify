...
namespace Spotify.Controllers
{
    public class SongController : Controller
    {
        ...
        public async Task<IActionResult> Detail(int id)
        {
            ...
            Song existSong = await _context.Songs.Include(m => m.ArtistSongs).ThenInclude(m => m.Artist).Where(m => m.Id == id && !m.IsDeleted).FirstOrDefaultAsync();

            SongDetailVM songDetailVM = new()
            {
            ...
            ViewBag.Song = await _context.Songs.Include(m => m.Comments).ThenInclude(m => m.AppUser).Where(m => m.Id == id && !m.IsDeleted).FirstOrDefaultAsync();

            ViewBag.CommentCount = await _context.Comments.Where(m => !m.IsDeleted).CountAsync();
        ...
        public async Task<IActionResult> AddWishlist(int? id)
        {
            ...
            Song song = await _context.Songs.FirstOrDefaultAsync(m => m.Id == id && !m.IsDeleted);
            ...
            IEnumerable<Wishlist> dbWishlists = await _context.Wishlists.Include(m => m.WishlistItems).Where(m => m.AppUserId == user.Id && !m.IsDeleted).ToListAsync();
                ...
                foreach (var dbWishlist in dbWishlists)
                {
                    foreach (var dbWishlistItem in dbWishlist.WishlistItems)
                    {
                        if (dbWishlistItem.SongId == id)
                        {
                            ...
                            songCount = await _context.WishlistItems.Where(m => m.Wishlist.AppUserId == user.Id && !m.IsDeleted).CountAsync();
